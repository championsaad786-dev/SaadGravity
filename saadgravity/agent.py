    ...
    def step(self, user_input: str):
        ...
        while current_step < self.max_steps:
            ...
            try:
                ...
                console.print(Panel(f"[bold red]LLM Generation Error:[/bold red] {str(e)}", border_style="red"))
                break
            ...
            if content and content.strip():
                ...
                clean_content = clean_content.replace("```tool_call", "").replace("```json", "")
                console.print(Panel(clean_content.strip(), title="[bold cyan]🧠 SaadGravity[/bold cyan]", border_style="cyan"))

            # If no tools requested, we are done
            if not tool_calls:
                # Add assistant response to history
                self.messages.append({"role": "assistant", "content": content})
                console.print("[dim green]✓ Task finished or awaiting your input.[/dim green]")
                break
            ...
            for tc in tool_calls:
                ...
                if not self.auto_execute:
                    ...
                    proceed = Confirm.ask(f"Execute [bold]{tool_name}[/bold]?", default=True)
                    if not proceed:
                ...
                output_preview = tool_output if len(tool_output) <= 800 else tool_output[:800] + f"\n... [truncated {len(tool_output)-800} chars]"
                console.print(Panel(
                    output_preview,
        ...
        if current_step >= self.max_steps:
            console.print("[bold red]⚠ Reached maximum autonomous step limit (25).[/bold red]")
