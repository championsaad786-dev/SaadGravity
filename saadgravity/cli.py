...
def print_banner(cfg: dict):
    ...
    auto_str = (
        ...
        else "OFF (Ask confirmation)"
    )
    status_table.add_row(
        "ONLINE", cfg.get("model", "N/A"), cfg.get("api_base", "N/A"), auto_str
    )
    console.print(status_table)
    console.print(
        "[dim]Type [bold white]/help[/bold white] for commands, [bold white]/model <name>[/bold white] to change model, [bold white]/exit[/bold white] to quit.\n[/dim]"
    )


def print_help():
    table = Table(
        title="[bold cyan]SaadGravity Command Reference[/bold cyan]", box=box.ROUNDED
    )
    table.add_column("Command", style="bold yellow", width=25)
    table.add_column("Description", style="white")

    table.add_row(
        "/model <name>", "Switch current LLM model (e.g. /model qwen2.5-coder:1.5b)"
    )
    table.add_row(
        "/backend <url>",
        "Change API endpoint (e.g. /backend http://localhost:11434/v1)",
    )
    table.add_row(
        "/key <apikey>",
        "Set custom API key for remote providers (OpenRouter, Groq, etc.)",
    )
    table.add_row(
        "/auto <on|off>", "Toggle autonomous tool execution without confirmation"
    )
...
def print_tools():
    table = Table(
        title="[bold cyan]Built-in Autonomous Tools[/bold cyan]", box=box.ROUNDED
    )
    table.add_column("Tool", style="bold green", width=22)
    table.add_column("Capability", style="white")

    table.add_row(
        "run_command", "Execute bash shell commands in Kali Linux with live outputs"
    )
    table.add_row(
        "view_file", "Inspect file contents with line numbering & range support"
    )
    table.add_row("write_file", "Create new code files or overwrite existing files")
    table.add_row(
        "replace_file_content", "Perform surgical search-and-replace on codebase"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="SaadGravity - Autonomous Terminal AI Coding Agent"
    )
    parser.add_argument(
        "prompt", nargs="*", help="Optional initial instruction to run directly"
    )
    parser.add_argument("-m", "--model", help="Specify model name to use")
    parser.add_argument(
        "-b", "--backend", help="Specify API Base URL (e.g. http://localhost:11434/v1)"
    )
    parser.add_argument("-k", "--key", help="Specify API Key")
    parser.add_argument(
        "--confirm",
        action="store_true",
        help="Ask confirmation before executing every tool",
    )
    ...
    while True:
        try:
            ...
            if user_input.startswith("/"):
                ...
                if cmd in ["/exit", "/quit"]:
                    console.print(
                        "[bold red]Exiting SaadGravity. Happy coding! 🚀[/bold red]"
                    )
                    break
                elif cmd == "/help":
                    print_help()
                    ...
                    console.print(
                        "[bold green]✓ Conversation context cleared.[/bold green]"
                    )
                elif cmd == "/status":
                    ...
                    if not arg:
                        console.print(
                            f"[yellow]Current model:[/yellow] {cfg.get('model')}"
                        )
                    else:
                        cfg = update_config_key("model", arg)
                        agent.set_model(arg)
                        console.print(
                            f"[bold green]✓ Model switched to:[/bold green] {arg}"
                        )
                elif cmd == "/backend":
                    if not arg:
                        console.print(
                            f"[yellow]Current backend:[/yellow] {cfg.get('api_base')}"
                        )
                    else:
                        cfg = update_config_key("api_base", arg)
                        agent.set_api_base(arg)
                        console.print(
                            f"[bold green]✓ Backend updated to:[/bold green] {arg}"
                        )
                elif cmd == "/key":
                    ...
                    state = (
                        "ENABLED (Auto-execute)" if val else "DISABLED (Manual confirm)"
                    )
                    console.print(
                        f"[bold green]✓ Autonomous execution {state}[/bold green]"
                    )
                else:
                    console.print(
                        f"[red]Unknown command:[/red] {cmd}. Type /help for options."
                    )
                continue
